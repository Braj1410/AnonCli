...
async function deposit({ currency, amount }) {
  ...
  const noteString = `anon-${currency}-${amount}-${netId}-${note}`;
  console.log(`Your note: ${noteString}`);
  if (currency === "eth") {
    await printETHBalance({ address: tornado._address, name: "Anon" });
    ...
    // a token
    await printERC20Balance({ address: tornado._address, name: "Anon" });
...
async function generateMerkleProof(deposit) {
  ...
  console.log("Getting current state from anon contract");
